...
input.addEventListener("input", () => {
    ...
    let patternEn = /[A-z#]+/;
    if (patternEn.test(letter) && ru[0].classList.contains("visVisible")) switchLang();
});
...
for (let i = 0; i < button.length; i++) {
    button[i].addEventListener("click", () => {
        ...
        // Смена языка
        if (
            (btn("ShiftLeft").classList.contains("checked") || btn("ShiftRight").classList.contains("checked")) &&
            (btn("AltLeft").classList.contains("checked") || btn("AltRight").classList.contains("checked"))
        ) {
            setTimeout(() => {
                switchLang();
